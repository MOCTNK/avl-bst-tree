...
public:
	void start();
	template <typename T> void create(MyTreeAVL<T>& myTree);
	template <typename T> void fillTree(MyTreeAVL<T>& myTree) {};
	template <typename T> void randomFillTree(MyTreeAVL<T>& myTree) {};
	template <typename T> void printTree(MyTreeAVL<T>& myTree) {};
	template <typename T> void printKeyList(MyTreeAVL<T>& myTree) {};
	template <typename T> void addByKey(MyTreeAVL<T>& myTree) {};
	template <typename T> void deleteNodeByKey(MyTreeAVL<T>& myTree) {};
	template <typename T> void updateValueByKey(MyTreeAVL<T>& myTree) {};
	template <typename T> void clearTree(MyTreeAVL<T>& myTree) {};
	template <typename T> void isTreeEmpty(MyTreeAVL<T>& myTree) {};
	template <typename T> void getValueByKey(MyTreeAVL<T>& myTree) {};
	template <typename T> void getBal(MyTreeAVL<T>& myTree) {};
	template <typename T> void getSize(MyTreeAVL<T>& myTree) {};
	template <typename T> void getMenuIterator(MyTreeAVL<T>& myTree) {};
	void menuIterator(typename MyTreeAVL<int>::Iterator& iterator);
	void menuReverseIterator(typename MyTreeAVL<int>::ReverseIterator& iterator);
	int getRandomNumber(int min, int max);
	int enterNumber(int min, int max);
	void testBigO(int n);
	void message(const char* mes);
	int searchBal(MyTreeAVL<int>& myTree, typename MyTreeAVL<int>::Node* r);
};
...
template <typename T> void Menu::create(MyTreeAVL<T>& myTree) {
	MyTreeAVL<T> tree = myTree;
	int action;
	while (true) {
		bool exit = false;
...
template <> void Menu::fillTree(MyTreeAVL<int>& myTree) {
	int size;
	while (true) {
...
template <> void Menu::randomFillTree(MyTreeAVL<int>& myTree) {
	int size;
	while (true) {
		cout << "Enter size:" << endl;
		size = enterNumber(0, 2147483647);
...
template <> void Menu::printTree(MyTreeAVL<int>& myTree) {
	if (!myTree.isEmpty()) {
		myTree.printTree(myTree.getRoot(), 1);
	}
	else {
...
template <> void Menu::getSize(MyTreeAVL<int>& myTree) {
	cout << "Size: " << myTree.getSize() << endl;
}

template <> void Menu::printKeyList(MyTreeAVL<int>& myTree) {
	myTree.printKeyList(myTree.getRoot());
}

template <> void Menu::addByKey(MyTreeAVL<int>& myTree) {
	int key;
	int value;
	while (true) {
		cout << "Enter key:" << endl;
...
template <> void Menu::deleteNodeByKey(MyTreeAVL<int>& myTree) {
	if (!myTree.isEmpty()) {
		int key;
		while (true) {
			cout << "Enter key:" << endl;
...
template <> void Menu::updateValueByKey(MyTreeAVL<int>& myTree) {
	if (!myTree.isEmpty()) {
		int value;
		int key;
		while (true) {
...
template <> void Menu::clearTree(MyTreeAVL<int>& myTree) {
	myTree.clearTree(myTree.getRoot());
	message("Tree cleared");
}

template <> void Menu::isTreeEmpty(MyTreeAVL<int>& myTree) {
	if (myTree.isEmpty()) {
...
template <> void Menu::getValueByKey(MyTreeAVL<int>& myTree) {
	if (!myTree.isEmpty()) {
		int key;
		while (true) {
			cout << "Enter key:" << endl;
...
template <> void Menu::getBal(MyTreeAVL<int>& myTree) {
	if (!myTree.isEmpty()) {
...
template <> void Menu::getMenuIterator(MyTreeAVL<int>& myTree) {
	if (!myTree.isEmpty()) {
		while (true) {
			bool exit = false;
			MyTreeAVL<int>::Iterator iter = myTree.begin();
			MyTreeAVL<int>::ReverseIterator rIter = myTree.rbegin();
			int action;
...
void Menu::menuIterator(typename MyTreeAVL<int>::Iterator& iterator) {
	...
	while (true) {
		...
		case 5:
			...
			break;
		case 6:
			message("Iterator is set to begin");
			iterator = iterator.begin();
			break;
		case 7:
			message("Iterator is set to end");
			iterator = iterator.end();
			break;
		case 8:
			exit = true;
...
void Menu::menuReverseIterator(typename MyTreeAVL<int>::ReverseIterator& iterator) {
	...
	while (true) {
		...
		case 5:
			...
			break;
		case 6:
			message("Reverse Iterator is set to begin");
			iterator = iterator.begin();
			break;
		case 7:
			message("Reverse Iterator is set to end");
			iterator = iterator.end();
			break;
		case 8:
			exit = true;
...
int Menu::searchBal(MyTreeAVL<int>& myTree, typename MyTreeAVL<int>::Node* r) {
	if (r == NULL) {
		return 0;
	}
